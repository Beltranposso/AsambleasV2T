...
export default function ProjectionViewProps {
  assembly: any
  onBack: () => void
}

export default function ProjectionView({ assembly, onBack }: ProjectionViewProps) {
  ...
  const startVoting = (votingData: any) => {
                        ...
                        {Object.entries(voting.votes).map(([option, count]) => (
                          <div key={option} className="text-center p-4 bg-white rounded-lg shadow-sm">
                            ...
                            <div className="text-xs text-gray-500">
                              {(((count as number) / voting.total) * 100).toFixed(1)}%
                            </div>
                          </div>
                        ))}
                      </div>
